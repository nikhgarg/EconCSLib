...
def allOnTypeAllocation {T : ℕ} (N : ℕ) (best : ItemType T) :
    CountAllocation T where
  count := fun t => if t = best then N else 0
...
theorem allOnTypeAllocation_total {T : ℕ}
  ...
  classical
  unfold EconCSLib.Allocation.total allOnTypeAllocation
  simp
...
theorem linearized_objective_eq_sum_count_mul_score {T : ℕ}
  ...
  unfold ConsumptionModel.objective EconCSLib.Allocation.objective
    ConsumptionModel.linearized ConsumptionModel.linearValueOfCount
...
theorem allOnTypeAllocation_linearized_isOptimalAtTotal {T : ℕ}
  ...
  · intro b hb
    have hsum_counts : (∑ t : ItemType T, (b.count t : ℝ)) = (N : ℝ) := by
      rw [← Nat.cast_sum]
      exact_mod_cast hb
    rw [linearized_objective_eq_sum_count_mul_score]
    rw [linearized_objective_eq_sum_count_mul_score]
    calc
      ...
      _ = ∑ t : ItemType T,
              ((allOnTypeAllocation N best).count t : ℝ) *
                (likelihood t * perItemValue t) := by
            symm
            unfold allOnTypeAllocation
            change
              (∑ t : ItemType T,
                ((if t = best then N else 0 : ℕ) : ℝ) *
                  (likelihood t * perItemValue t)) =
                (N : ℝ) * (likelihood best * perItemValue best)
            simp
...
theorem linearized_optimal_count_eq_zero_of_strict_score_lt
    ...
    a.count t = 0 := by
  by_contra hne_zero
  have hpos : 0 < a.count t := Nat.pos_of_ne_zero hne_zero
  have ht_ne_best : t ≠ best := by
    intro h
    rw [h] at hstrict
    exact (lt_irrefl (likelihood best * perItemValue best)) hstrict
  have hle :=
    ConsumptionModel.weightedForwardMarginal_le_weightedBackwardMarginal_of_optimum
        (M := ConsumptionModel.linearized likelihood perItemValue)
        (N := N) (a := a) (src := t) (dst := best)
        hopt ht_ne_best hpos
  have hforward :
      ConsumptionModel.weightedForwardMarginal
          (ConsumptionModel.linearized likelihood perItemValue)
          best (a.count best) =
        likelihood best * perItemValue best := by
    unfold ConsumptionModel.weightedForwardMarginal ConsumptionModel.marginalValue
      EconCSLib.Allocation.marginal ConsumptionModel.linearized
      ConsumptionModel.linearValueOfCount
    change likelihood best *
        (((a.count best + 1 : ℕ) : ℝ) * perItemValue best -
          (a.count best : ℝ) * perItemValue best) =
      likelihood best * perItemValue best
  ...
  have hbackward :
    ...
    rw [dif_neg hpos.ne']
    change likelihood t *
        ((a.count t : ℝ) * perItemValue t -
          ((a.count t - 1 : ℕ) : ℝ) * perItemValue t) =
      likelihood t * perItemValue t
