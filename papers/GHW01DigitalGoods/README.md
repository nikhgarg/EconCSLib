...
| Theorem 7.1, weighted pairing auction revenue | `weightedPairingExpectedRevenue`, `paper_theorem7_1_weighted_pairing_from_factor_two_ranked_bins_of_value_bounds` | conditional | `GHW01DigitalGoods/MainTheorems.lean` | Weighted-pairing expected-payment/revenue formula, singleton-bin deletion algebra, per-bin constant algebra, Cauchy square-sum bridge, factor-two ranked-bin geometry, same-bin probability bound, and embedding of same-bin ranked double sums into the concrete weighted-pairing revenue are formalized; remaining proof boundary is constructing the paper's dyadic active-bin ranking/partition certificate and log-`h` bin count from an arbitrary bid vector |
