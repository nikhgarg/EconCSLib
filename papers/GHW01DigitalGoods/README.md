...
| Theorem 7.2, weighted auction benchmark bound | `paper_theorem7_2_weighted_pairing_bound_for_two_winner_benchmark`, `paper_theorem7_2_tightness_ratio_certificate` | formalized with caveat | `GHW01DigitalGoods/MainTheorems.lean` | The main lower bound is closed for the finite two-winner fixed-price benchmark: if bids lie in `[1, 2^(m+1)]`, `F^(2) >= 2 * 2^(m+1)`, `2 <= s`, and `(m+1 : R) <= s^2`, then `F^(2) <= 576 * s * W`. The proof includes the first/second case split, branch-local `4h <= T`, internally constructed power-of-two dyadic bins, canonical dyadic fixed-price tail buckets, selected-largest bucket construction, the strict finite geometric-tail proof that the largest bucket has at least two bidders, the high/low rank split, and the concrete weighted-pairing revenue bridge. Tightness is formalized as a contribution/ratio certificate `W <= (3/s)F`; the remaining caveat is instantiating that certificate with the paper's concrete repeated-bid family inside the weighted-pairing expectation model. |
...
| Theorem 9.3, deterministic truthful lower bound | `paper_theorem9_3_deterministic_truthful_lower_bound_of_count_threshold`, `paper_theorem9_3_deterministic_truthful_lower_bound_of_count_threshold_fixed_price_benchmark` | conditional | `GHW01DigitalGoods/MainTheorems.lean` | Current status: conditional scaffold; reduction from count-threshold bid-independent rule to the Theorem 9.1 lower-bound witness is formalized both as an abstract lower-bound certificate and against the actual one-winner finite fixed-price benchmark; Lemma 9.2 auction-level slices are closed; needs conversion from general critical-price slices to the count-threshold model |
