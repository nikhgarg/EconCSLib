...
This folder is a public partial formalization. The reusable auction primitives,
benchmark support, and many named theorem endpoints are formalized, but the
paper is not marked fully formalized because several final endpoints still take
explicit model-certificate assumptions. The next full-formalization pass should
derive the Theorem 8.2 anonymous sorted-bid certificate and the Theorem 9.3
anonymous erased-bid critical-price certificate from the paper's primitive
truthfulness/anonymity assumptions, or make those assumptions explicit in the
...
| Theorem 4.1 and Corollary 4.2, fixed-price revenue lower bounds | `paper_theorem4_1_finite_candidate_benchmark_from_power_two_bins`, `paper_theorem4_1_finite_candidate_benchmark_from_log_certificate`, `paper_theorem4_1_finite_candidate_benchmark_from_logb_high_value`, `PaperTheorem41HighValueModel`, `paper_theorem4_1_finite_candidate_benchmark_of_high_value_model`, `paper_corollary4_2_fixed_price_lower_bound_from_truncation`, `PaperCorollary42TruncationModel`, `paper_corollary4_2_fixed_price_lower_bound_of_truncation_model` | conditional | `GHW01DigitalGoods/MainTheorems.lean` | Theorem 4.1 is closed for the finite one-winner candidate fixed-price benchmark with `Real.logb 2 h + 2`; Corollary 4.2 is still packaged through `PaperCorollary42TruncationModel` rather than derived directly from the paper's truncation setup. |
...
8.2 and 9.3 now both have conditional paper-model theorems: their model fields
make the paper's anonymous sorted-bid and anonymous erased-bid critical-price
conventions explicit, but those fields still need constructors from primitive
source assumptions before the paper can be marked fully formalized.

## Handoff Notes

- Last passing paper build: `lake build GHW01DigitalGoods`.
- Current status: public partial formalization. Treat this as the next paper to
  try to finish fully by discharging the model-certificate assumptions above.
