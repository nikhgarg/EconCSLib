...
| Theorem 4.1 and Corollary 4.2, fixed-price revenue lower bounds | `paper_theorem4_1_fixed_price_lower_bound_from_power_two_bins`, `paper_corollary4_2_fixed_price_lower_bound_from_truncation` | formalized with caveat | `GHW01DigitalGoods/MainTheorems.lean` | Theorem 4.1 is closed in the natural-number dyadic form: if bids lie in `[1, 2^(m+1)]`, then `T <= 2 * (m+1) * F`. The proof constructs the power-of-two classifier bins internally and uses an empty-bin-safe factor-two partition lemma. Corollary 4.2 truncation algebra is formalized; remaining caveat is only the exact paper `Real.log h` / `Real.log n` notation bridge. |
