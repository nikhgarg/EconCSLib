...
| Theorem 7.1, weighted pairing auction revenue | `weightedPairingExpectedRevenue`, `paper_theorem7_1_weighted_pairing_log_bound_of_finset_partition` | conditional | `GHW01DigitalGoods/MainTheorems.lean` | Weighted-pairing expected-payment/revenue formula, singleton-bin deletion algebra, per-bin constant algebra, Cauchy square-sum bridge, factor-two ranked-bin geometry, same-bin probability bound, embedding of same-bin ranked double sums into the concrete weighted-pairing revenue, log-bin-count algebra, singleton floor-sum deletion, and automatic value-sorted finset-bin ranking/injectivity are formalized; remaining proof boundary is constructing the paper's dyadic active/singleton finset partition and log-count certificate from an arbitrary bid vector |
| Theorem 7.2, weighted auction benchmark bound | `paper_theorem7_2_first_case_from_theorem7_1`, `paper_theorem7_2_second_case_bucket_split_finsets` | conditional | `GHW01DigitalGoods/MainTheorems.lean` | First case from Theorem 7.1 (`F <= T/s`), second-case high-low bucket selected-pair revenue certificate, split-size cross-mass algebra, and disjoint high/low finset wrapper with internal pair-injection proof are formalized; remaining proof boundary is constructing the paper's largest-bucket split certificate and the tightness example |
