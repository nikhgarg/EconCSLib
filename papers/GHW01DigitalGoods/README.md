...
| Theorem 4.1 and Corollary 4.2, fixed-price revenue lower bounds | `paper_theorem4_1_finite_candidate_benchmark_from_power_two_bins`, `paper_theorem4_1_finite_candidate_benchmark_from_log_certificate`, `paper_corollary4_2_fixed_price_lower_bound_from_truncation` | formalized with caveat | `GHW01DigitalGoods/MainTheorems.lean` | Theorem 4.1 is closed in the natural-number dyadic form for the finite one-winner candidate fixed-price benchmark: if bids lie in `[1, 2^(m+1)]`, then `T <= 2 * (m+1) * F`; the log-certificate wrapper gives `T <= 2 * logH * F` from `(m+1 : R) <= logH`. The proof constructs the power-of-two classifier bins internally, proves price `1` feasible under the paper normalization, and uses an empty-bin-safe factor-two partition lemma. Corollary 4.2 truncation algebra is formalized; remaining caveat is only the exact paper `Real.log h` / `Real.log n` notation bridge. |
...
| Theorem 7.2, weighted auction benchmark bound | `paper_theorem7_2_weighted_pairing_bound_for_two_winner_benchmark`, `paper_theorem7_2_tightness_ratio_certificate`, `paper_theorem7_2_tightness_revenue_split_by_classifier`, `paper_theorem7_2_tightness_ratio_from_classifier`, `GhwTightAgent`, `ghwTightValue`, `paper_theorem7_2_tightness_totalBidValue`, `paper_theorem7_2_tightness_singlePriceRevenue_top`, `paper_theorem7_2_tightness_top_revenue_le_twoWinnerBenchmark` | formalized with caveat | `GHW01DigitalGoods/MainTheorems.lean` | The main lower bound is closed for the finite two-winner fixed-price benchmark: if bids lie in `[1, 2^(m+1)]`, `F^(2) >= 2 * 2^(m+1)`, `2 <= s`, and `(m+1 : R) <= s^2`, then `F^(2) <= 576 * s * W`. The proof includes the first/second case split, branch-local `4h <= T`, internally constructed power-of-two dyadic bins, canonical dyadic fixed-price tail buckets, selected-largest bucket construction, the strict finite geometric-tail proof that the largest bucket has at least two bidders, the high/low rank split, and the concrete weighted-pairing revenue bridge. Tightness now defines the repeated-bid family, proves each lower level has total mass `2^k`, proves total bid value `2^k(s+k-1)`, proves the top price earns `s*2^k` and is feasible for the actual two-winner benchmark, and proves the concrete weighted-pairing revenue splits by a lower-level/top-level classifier before applying contribution bounds. Remaining caveat: prove the paper's numerical lower/top contribution bounds for this family from the raw weighted-pairing formula. |
...
ranked-bin weighted-pairing bridge, the closed Theorem 4.1 and Theorem 7.2
natural-number dyadic forms, Lemma 8.1, and Lemma 9.2. The remaining open
endpoints are exact `Real.log` notation packaging for Sections 4 and 7, the
Section 7.2 concrete tightness-family instantiation, and the Section 8--9
general-auction wrappers listed above.
