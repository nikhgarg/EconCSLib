...
| Theorem 7.1, weighted pairing auction revenue | `weightedPairingExpectedRevenue`, `paper_theorem7_1_bin_revenue_lower_bound_algebra`, `paper_theorem7_1_weighted_pairing_from_active_bin_certificates` | conditional | `GHW01DigitalGoods/MainTheorems.lean` | Weighted-pairing expected-payment/revenue formula, singleton-bin deletion algebra, per-bin constant algebra, Cauchy square-sum bridge, and final active-bin certificate are formalized; remaining proof boundary is deriving the raw per-bin revenue lower bounds from the weighted-pairing double-sum model and concrete dyadic bins |
...
paper's top-prefix exponential constants, Lemma 8.1, and Lemma 9.2. The source
