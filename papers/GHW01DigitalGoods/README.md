...
| Theorem 7.2, weighted auction benchmark bound | `paper_theorem7_2_weighted_pairing_bound_from_classifier_and_geometric_tail_certificates`, `paper_theorem7_2_tightness_contribution_certificate` | conditional | `GHW01DigitalGoods/MainTheorems.lean` | First/second case split, first case from Theorem 7.1 (`F <= T/s`), classifier-bin derivation of the Section 7.1 premise, second-case high-low selected-pair revenue certificate, rank-split bucket halves, canonical finite dyadic geometric-tail proof of `\|M\|t = Ω(F)`, fixed-price-to-tail bridge, tail-set inclusion bridge, largest-total-bucket floor-mass bridge, concrete `F <= 576 s W` certificate, and tightness-example summation algebra are formalized; remaining proof boundary is instantiating the dyadic classifier, geometric tail/bucket certificates, bucket-size fact, and concrete tightness example from an arbitrary bid vector/family |
