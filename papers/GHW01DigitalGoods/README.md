...
| Theorem 7.1, weighted pairing auction revenue | `weightedPairingExpectedRevenue`, `paper_theorem7_1_weighted_pairing_log_bound_of_active_singleton_bins` | conditional | `GHW01DigitalGoods/MainTheorems.lean` | Weighted-pairing expected-payment/revenue formula, singleton-bin deletion algebra, per-bin constant algebra, Cauchy square-sum bridge, factor-two ranked-bin geometry, same-bin probability bound, embedding of same-bin ranked double sums into the concrete weighted-pairing revenue, log-bin-count algebra, singleton floor-sum deletion, automatic value-sorted finset-bin ranking/injectivity, and active/singleton splitting from one finite bin family are formalized; remaining proof boundary is constructing the paper's dyadic bin family and log-count certificate from an arbitrary bid vector |
| Theorem 7.2, weighted auction benchmark bound | `paper_theorem7_2_weighted_pairing_bound_from_tail_set_certificate` | conditional | `GHW01DigitalGoods/MainTheorems.lean` | First/second case split, first case from Theorem 7.1 (`F <= T/s`), second-case high-low selected-pair revenue certificate, rank-split bucket halves, finite geometric-tail proof of `\|M\|t = Ω(F)`, fixed-price-to-tail bridge, tail-set inclusion bridge, largest-total-bucket floor-mass bridge, and concrete `F <= 576 s W` certificate are formalized; remaining proof boundary is instantiating the dyadic tail/bucket certificates and bucket-size fact from an arbitrary bid vector, plus the tightness example |
