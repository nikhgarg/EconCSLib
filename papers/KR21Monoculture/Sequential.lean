...
@[simp] theorem shared_bestInSet_eq {n : ℕ} (π : Ranking n)
  ...
  unfold EconCSLib.SocialChoice.Ranking.bestInSet bestInSet
  by_cases h : remaining.Nonempty
  · simp only [dif_pos h]
    change
      π ((Finset.image (fun c : Candidate n => π.symm c) remaining).min' _) =
        π ((Finset.image (fun c : Candidate n => π.symm c) remaining).min' _)
    rfl
  · simp [h, firstChoice, EconCSLib.SocialChoice.Ranking.firstChoice]
...
theorem le_castSucc_of_le_succ_of_ne
    ...
    x ≤ k.castSucc := by
  exact EconCSLib.SocialChoice.Ranking.le_castSucc_of_le_succ_of_ne hx hne

theorem succ_le_of_castSucc_le_of_ne
    {n : ℕ} {k : Fin (n + 1)} {x : Fin (n + 2)}
    (hx : k.castSucc ≤ x) (hne : x ≠ k.castSucc) :
    k.succ ≤ x := by
  exact EconCSLib.SocialChoice.Ranking.succ_le_of_castSucc_le_of_ne hx hne
...
theorem swapInversionMap_mem_inversionFinset_refl
  ...
  · rw [if_pos hbi]
    ...
    have htarget :
        invertedPair (Equiv.refl (Candidate n)) τ (a, j) := by
      exact ⟨by simpa [rankOf] using haj, hlt_rank⟩
  ...
  · rw [if_neg hbi]
    ...
    · rw [if_pos haj_eq]
      ...
      have htarget :
          invertedPair (Equiv.refl (Candidate n)) τ (i, b) := by
        exact ⟨by simpa [rankOf] using lt_trans hij hjb, hlt_rank⟩
      simpa [inversionFinset] using htarget
    · rw [if_neg haj_eq]
      by_cases hai_jb : a = i ∧ j < b
      · rw [if_pos hai_jb]
        ...
        have hlt_rank : rankOf τ b < rankOf τ j := by
          simpa [hrank_b] using hrank
        have htarget :
            invertedPair (Equiv.refl (Candidate n)) τ (j, b) := by
          exact ⟨by simpa [rankOf] using hjb, hlt_rank⟩
        simpa [inversionFinset] using htarget
      · rw [if_neg hai_jb]
        by_cases hbj_ai : b = j ∧ a < i
        · rw [if_pos hbj_ai]
          ...
          have hlt_rank : rankOf τ i < rankOf τ a := by
            simpa [hrank_a] using hrank
          have htarget :
              invertedPair (Equiv.refl (Candidate n)) τ (a, i) := by
            exact ⟨by simpa [rankOf] using hai, hlt_rank⟩
          simpa [inversionFinset] using htarget
        · rw [if_neg hbj_ai]
          by_cases hai : a = i
          · subst a
            ...
            have htarget :
                invertedPair (Equiv.refl (Candidate n)) τ (i, b) := by
              exact ⟨by simpa [rankOf] using hib, hlt_rank⟩
            simpa [inversionFinset] using htarget
          · by_cases hbj : b = j
            · subst b
              have haj_lt : a < j := hab
              ...
              have htarget :
                  invertedPair (Equiv.refl (Candidate n)) τ (a, j) := by
                exact ⟨by simpa [rankOf] using haj_lt, hlt_rank⟩
              simpa [inversionFinset] using htarget
            · have ha_ne_i : a ≠ i := hai
              ...
              have htarget :
                ...
                exact ⟨by simpa [rankOf] using hab, hlt_rank⟩
              simpa [inversionFinset] using htarget
...
theorem kendallTau_refl_swapCandidatePositions_lt
  ...
  have hpair_mem : (i, j) ∈ t := by
    have hinv : invertedPair (Equiv.refl (Candidate n)) τ (i, j) := by
      exact ⟨by simpa [rankOf] using hij, hpos⟩
...
theorem ranking_eq_refl_of_no_adjacent_inversions
  ...
  have hmono : StrictMono π := by
    ...
    intro k
    have hle : π k.castSucc ≤ π k.succ := by
      exact le_of_not_gt (by simpa [rankOf] using hno k)
...
noncomputable def rankingPairLeftExp {n : ℕ}
    (γ : PMF (Ranking n × Ranking n)) (F : Ranking n → ℝ) : ℝ :=
  ∑ z : Ranking n × Ranking n, (γ z).toReal * F z.1

/-- Right-coordinate expectation under a coupling of two rankings. -/
noncomputable def rankingPairRightExp {n : ℕ}
    (γ : PMF (Ranking n × Ranking n)) (F : Ranking n → ℝ) : ℝ :=
  ∑ z : Ranking n × Ranking n, (γ z).toReal * F z.2
...
theorem firstChoiceTailRemainingOf_castSucc_eq_succ_of_not_mem
  ...
  by_cases hc : c = k
  · subst c
    have hleft : k.castSucc.succAbove k = k.succ := by
      exact Fin.succAbove_castSucc_self k
  ...
  · have hsucc_ne_zero : k.succ ≠ (0 : Candidate (n + 1)) :=
    ...
    have hsame :
      ...
      by_cases hlt : c < k
      · have hsucc_above :
            k.succ.succAbove c = c.castSucc := by
          exact Fin.succAbove_succ_of_le k c (le_of_lt hlt)
        have hcast_above :
            k.castSucc.succAbove c = c.castSucc := by
          exact Fin.succAbove_castSucc_of_lt k c hlt
        rw [hsucc_above, hcast_above]
      · have hkc : k < c := by
          exact lt_of_le_of_ne (le_of_not_gt hlt) (Ne.symm hc)
        have hsucc_above :
            k.succ.succAbove c = c.succ := by
          exact Fin.succAbove_succ_of_lt k c hkc
        have hcast_above :
            k.castSucc.succAbove c = c.succ := by
          exact Fin.succAbove_castSucc_of_le k c (le_of_lt hkc)
        rw [hsucc_above, hcast_above]
...
theorem swapCandidatePositions_rankingFirstChoiceOrderEquiv_succAbove
  ...
  · subst e
    ...
    have hleft :
        rankOf (rankingFirstChoiceOrderEquiv n (r, σ)) r = 0 := by
      exact (rankOf_eq_zero_iff_eq_firstChoice
    ...
    have hright :
        rankOf
            (rankingFirstChoiceOrderEquiv n
              (r, swapCandidatePositions σ c d)) r = 0 := by
      exact (rankOf_eq_zero_iff_eq_firstChoice
        (rankingFirstChoiceOrderEquiv n
...
theorem weaklyOrderedBy_succAbove
  ...
  intro c d hcd
  have hlt : r.succAbove c < r.succAbove d := by
    exact (Fin.succAbove_lt_succAbove_iff).mpr (by simpa [rankOf] using hcd)
...
theorem weaklyOrderedBy_castSucc
  ...
  intro c d hcd
  have hlt : c.castSucc < d.castSucc := by
    exact Fin.castSucc_lt_castSucc_iff.mpr (by simpa [rankOf] using hcd)
  exact hvalue (by simpa [rankOf] using hlt)
...
theorem succAbove_pred_castSucc_eq_of_ne
  ...
  · have hi_lt_k : i.castSucc < k := by
    ...
    have hi_lt_prev : i.castSucc < kp.castSucc := by
...
theorem rankingPeelBestOrderEquiv_adjacent_swap
  ...
  have hprev_ne_k : prev ≠ k := by
    have hk_eq : k = (k.pred hk).succ := by
      exact (Fin.succ_pred k hk).symm
    rw [hk_eq]
    exact ne_of_lt (by
      dsimp [prev]
...
theorem bestInSet_rankingPeelBestOrderEquiv_of_zero_not_mem
  ...
  · intro e he
    ...
    | succ a =>
        ...
        have hlt_tail :
            rankOf σ a <
              rankOf σ (bestInSet σ (tailRemainingOf remaining)) := by
  ...
  · intro e he
    ...
    | succ a =>
        ...
        have hlt_full :
          ...
          lt_of_not_ge hnot
        have hlt_tail : rankOf σ a < rankOf σ b := by
          exact (rankOf_rankingPeelBestOrderEquiv_succ_lt_iff
...
theorem bestInSet_rankingPeelWorstOrderEquiv_of_last_not_mem
  ...
  · intro e he
    ...
    have hlt_init :
        rankOf σ a <
          rankOf σ (bestInSet σ (initRemainingOf remaining)) := by
...
theorem bestInSet_rankingPeelWorstOrderEquiv_of_last_mem_tail_before
  ...
  · intro e he
    ...
    · rcases Fin.eq_castSucc_of_ne_last
      ...
      subst e
      have ha_mem : a ∈ initRemainingOf remaining := by
        exact (mem_initRemainingOf (remaining := remaining)
          (c := a)).mpr he
      have hb_le_a : rankOf σ b ≤ rankOf σ a :=
...
theorem weaklyOrderedBy_centerPrefixValue {n : ℕ} (k : Fin (n + 1)) :
  ...
  · have hc : c ≤ k.castSucc := by
      exact le_trans (le_of_lt (by simpa [rankOf] using hcd)) hd
    simp [hc, hd]
...
theorem reflMallowsPayoffSum_cross_bestInSet_of_prefix
  ...
  intro k _
  have hgap : 0 ≤ gap k := by
    exact sub_nonneg.mpr (hvalue (by
          ...
          · have hbpos_nat : 0 < b := Nat.lt_of_le_of_lt (Nat.zero_le a) hab
            ...
            have hright :
                0 ≤ C ⟨b, hb⟩ * S ⟨k, hk_lt_m⟩ -
                  C ⟨k, hk_lt_m⟩ * S ⟨b, hb⟩ := by
              exact hadjC ⟨k, hk_lt_m⟩ ⟨b, hb⟩ hk_succ.symm
            have hsum :
                0 ≤
                  C ⟨b, hb⟩ *
                      (C ⟨k, hk_lt_m⟩ * S ⟨a, ha⟩ -
...
noncomputable def candidateRankInitialPowerSum
  ...
  ∑ i : Candidate n, if i ≤ k then q ^ (i : ℕ) else 0
...
noncomputable def candidateRankStrictSuffixReversePowerSum
    (n : ℕ) (q : ℝ) (k : Candidate n) : ℝ :=
  ∑ i : Candidate n, if k < i then q ^ (n + 1 - (i : ℕ)) else 0
...
theorem firstChoiceBranchPayoffSum_prefixCut_eq_of_adjacent_not_mem
  ...
  · by_cases hcast_cut : ((k.castSucc : Candidate (n + 1)) : ℕ) < cut
    · have hcut_eq : cut = ((k.succ : Candidate (n + 1)) : ℕ) := by
        ...
        omega
      have htail_nonempty :
          (firstChoiceTailRemainingOf k.succ remaining).Nonempty := by
        exact
          firstChoiceTailRemainingOf_nonempty_of_nonempty_of_first_not_mem
            hremaining hsucc
      have hk_tail :
    ...
    have hright_ge : cut ≤ ((k.succ : Candidate (n + 1)) : ℕ) := by
      have hcast_le_succ :
          ((k.castSucc : Candidate (n + 1)) : ℕ) ≤
            ((k.succ : Candidate (n + 1)) : ℕ) := by
        exact Nat.le_of_lt k.castSucc_lt_succ
      omega
    rw [
      firstChoiceBranchPayoffSum_prefixCut_eq_zero_of_mem_ge
...
theorem ranking_zero_eq_of_firstChoice {π σ : Ranking 0}
  ...
  · have hπ_ne_first : π 1 ≠ firstChoice π := by
      intro hπ
      have hidx : (1 : Candidate 0) = 0 := by
        exact π.injective (by simpa [firstChoice] using hπ)
      norm_num at hidx
    have hσ_ne_first : σ 1 ≠ firstChoice σ := by
      intro hσ
      have hidx : (1 : Candidate 0) = 0 := by
        exact σ.injective (by simpa [firstChoice] using hσ)
      norm_num at hidx
    by_cases hπ0 : firstChoice π = (0 : Candidate 0)
    · have hσ0 : firstChoice σ = (0 : Candidate 0) := by
        exact h.symm.trans hπ0
    ...
    · have hπfirst1 : firstChoice π = (1 : Candidate 0) :=
      ...
      have hσfirst1 : firstChoice σ = (1 : Candidate 0) := by
        exact h.symm.trans hπfirst1
...
noncomputable def rankingZeroFirstChoiceEquiv : Ranking 0 ≃ Candidate 0 where
  ...
  right_inv c := by
    exact firstChoice_rankingZeroOfFirstChoice c

@[simp] theorem rankingZeroFirstChoiceEquiv_symm_apply (c : Candidate 0) :
    rankingZeroFirstChoiceEquiv.symm c = rankingZeroOfFirstChoice c :=
  rfl
...
noncomputable def rankLayerCountThree (r : Candidate 2) : ℝ :=
  if r = (0 : Candidate 2) ∨ r = (3 : Candidate 2) then 1 else 2
    ...
    0 ≤
      (∑ j : Candidate 2, rankLayerCountThree j * qLess ^ (j : ℕ)) *
        ...
        (∑ j : Candidate 2, rankLayerCountThree j * qMore ^ (j : ℕ)) *
          (∑ i : Candidate 2,
            (rankLayerCountThree i * qLess ^ (i : ℕ)) * B i) := by
  exact candidateWeightedAverage_cross_nonneg_of_pairwise
    2
...
noncomputable def rankingOneOfFirstTail
    (r : Candidate 1) (t : Candidate 0) : Ranking 1 :=
  rankingFirstChoiceOrderEquiv 0 (r, rankingZeroOfFirstChoice t)
...
theorem rankingOneOfFirstTail_apply_tail
    ...
    rankingOneOfFirstTail r t
      ...
      r.succAbove c := by
  exact apply_eq_of_rankOf _ (rankOf_rankingOneOfFirstTail_succAbove r t c)
  ...
  | succ ctail =>
      ...
      | succ dtail =>
          have htail :
            ...
            tailRemainingOf_nonempty_of_nonempty_of_zero_not_mem
              hremaining hzero
          have hc_tail : ctail ∈ tailRemainingOf remaining := by
            exact (mem_tailRemainingOf (remaining := remaining)
              (c := ctail)).mpr hc
          have hd_tail : dtail ∈ tailRemainingOf remaining := by
            exact (mem_tailRemainingOf (remaining := remaining)
              (c := dtail)).mpr hd
...
theorem reflMallowsBestInSetWeight_cross_nonneg_of_last_not_mem
  ...
  have hinit :
    ...
    initRemainingOf_nonempty_of_nonempty_of_last_not_mem
      hremaining hlast
  have hc_init : ctail ∈ initRemainingOf remaining := by
    exact (mem_initRemainingOf (remaining := remaining)
      (c := ctail)).mpr hc
  have hd_init : dtail ∈ initRemainingOf remaining := by
    exact (mem_initRemainingOf (remaining := remaining)
      (c := dtail)).mpr hd
  have hcd_init : ctail < dtail := by
    exact Fin.castSucc_lt_castSucc_iff.mp hcd
  have hinit_cross :=
...
theorem reflMallowsPayoffSum_cross_bestInSet_univ_of_q_lt
    ...
    0 ≤
      mallowsPartition qLess (Equiv.refl (Candidate n)) *
        ...
        mallowsPartition qMore (Equiv.refl (Candidate n)) *
          reflMallowsPayoffSum n qLess
            (fun τ : Ranking n => value (bestInSet τ Finset.univ)) := by
  exact
...
def remainingAfter (hired : Finset (Candidate n)) : Finset (Candidate n) :=
  Finset.univ \ hired
...
def allHumanSequence (k : ℕ) : StrategySequence k :=
  fun _ => Strategy.human
