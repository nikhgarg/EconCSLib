...
| Lemma 5, locally envy-free equilibrium gives stable assignment | `paper_position_slot_envy_free_no_profitable_assigned_slot_deviation`, `paper_position_slot_envy_free_stable_assignment`, `paper_position_locally_envy_free_equilibrium_stable_assignment` | conditional | `EOS07GSP/MainTheorems.lean` | Current Lean bridge proves stability from feasibility, individual rationality, and the formal all-assigned-slot `SlotEnvyFree` predicate. The paper's appendix supplies the missing adjacent-LEF-to-global-no-rematch route via equilibrium/no-tie, assortative ordering, and telescoped adjacent inequalities; that source derivation remains to be formalized generically. |
| Lemma 6, stable assignment gives locally envy-free outcome | `paper_position_stable_assignment_slot_envy_free` | conditional | `EOS07GSP/MainTheorems.lean` | Current Lean bridge proves the outcome-level formal implication because `StableAssignment` already contains the no-profitable-rematch inequalities. The paper includes the `N > K` hypothesis and invokes the Shapley-Shubik stable-assignment characterization before constructing GSP bids; that characterization plus the bid-construction/static-equilibrium converse remains to be linked. |
