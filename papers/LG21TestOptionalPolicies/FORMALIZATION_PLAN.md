...
3. Build an observed-access threshold best-response interface for Lemma 4.1
   ...
   consuming fully specified threshold source models, where the lower-tail
   no-report and no-test estimates are also definitional.  The remaining
   strategic bridge is to prove that those fully specified threshold source
   games are the concrete source equilibria generated by the paper's Bayesian
   policy.
