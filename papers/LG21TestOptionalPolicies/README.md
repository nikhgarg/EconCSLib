...
- Compact human-review theorem ledger: `LG21TestOptionalPolicies/PaperInterface.lean`
  (16 paper-facing rows; proof-route variants stay out of this file)
...
- Standalone source-surface audit: `SOURCE_AUDIT.md`
...
- The source-shaped PMF and continuous-law witness routes now also have direct
  ...
  and non-Section fairness/test-blank and fairness/no-relevance iff wrappers
...
- Theorem 3.2 now also has PMF and continuous-law iff wrappers that combine
  ...
  Section 3 variants bundling the hidden-access hypothesis.  The same PMF/law
  and Section 3 routes are now also exposed through
...
- The event-share case split now has a share-language bridge:
  ...
  nonzero reporter/taker-share profiles; the positive-event variants
  ...
  state it directly for positive-mass reporter/taker atoms.
  ...
  The strengthened iff endpoints
  `paper_interface_definition5_blank_on_zero_event_share_no_normalized_relevance_iff_no_raw_relevance_on_nonzero_share`
  and
  `paper_interface_definition5_blank_on_zero_event_share_no_normalized_relevance_iff_no_raw_relevance_on_positive_event`
...
- The paper-facing final Theorem 3.2 aliases are now
  ...
  directly as zero reporter/taker event share implies test-blankness, while
  the `*_pbo_threshold_literal_event` aliases expose the paper's `P_BO`
  threshold notation and discharge that branch from full finite support plus a
  literal reporter/taker event.
...
- `paper_theorem3_2_observable_fair_best_response_forces_no_above_mean_actor`
  ...
  algebra by construction for that normalized affine source model.  The
  ...
  selected tests.  The literal finite-test mapped-actor routes now also have
  support-aware endpoints:
  ...
  rule out distinct positive-mass test estimates under fairness, while
  ...
  state the corresponding support-aware full-feature/base-only PMF equality;
  the `paper_interface_theorem3_2_no_positive_mass_test_relevance_of_fair_*`
  aliases package this as the absence of any positive-mass relevance witness.
  The finite PMF raw-mixture route now also has a
...
short Section 3 paper-facing aliases; the strongest current routes derive
source witnesses and upper-tail impossibility from concrete source equilibria
under explicit payoff, tie-breaking, threshold, mixture, and finite
event-support hypotheses. The `P_BO` threshold wrappers
now convert the paper's Bayesian-optimal notation to cutoff decisions in both
optional-reporting and report-required regimes; their full-support
literal-event variants derive the old zero-share blankness branch from the
displayed positive-support premise instead of requiring it separately, and the
new source-equilibrium bundles carry the `P_BO` threshold notation through the
continuous-law unfairness endpoints for the no-report/no-take mixture,
finite-event-share, and every-equilibrium variants. The literal-decision/event
variants make the finite reporter/taker event and
the reporting/taking decision definitional `P_BO` threshold objects. The
support-aware `P_BO` wrappers
...
now route the Gaussian/affine Bayesian-optimal threshold comparisons directly
to the positive-mass finite-test no-relevance endpoints, without a global full
support premise over zero-mass tests. Their literal-decision companions accept
the source-equilibrium reporting/taking decision itself as the `P_BO` threshold
comparison and internally rewrite it to the induced cutoff. Their actor-support
companions derive the finite-test support premise from the pushforward acting
law's positive-mass support, aligning this route with the corrected
base-source-model support hypotheses. The
Theorem 3.1 `P_BO` source-witness endpoints now separately prove the pure
strategic-withholding conclusion directly from the real-valued
Bayesian-optimal threshold rule, so finite student-support premises are only
needed for the distributional event-share surfaces, not for the cutoff witness
itself. The same `P_BO` source-witness route is now bundled with the
source-shaped PMF unfairness surfaces, and the full-support PMF certificate
route now has explicit `P_BO` fact, certificate-family, and direct
Section 3 PMF strategic-withholding aliases for both policy regimes. The
nonempty-equilibrium `P_BO` diagnostic wrappers now also connect the same
Bayesian-optimal notation directly to the upper-tail source-model
inconsistency, so those diagnostic endpoints need neither a zero-share
blankness premise nor full finite event support. The source-equilibrium
...
also removes the slope parameter. The repaired a.e. continuous-law route now
has Gaussian marginal-law wrappers and compact certificate contradictions:
...
These are the preferred endpoints for continuous threshold models with null
cutoff/boundary types. The compact paper-facing a.e. no-relevance and
fairness-impossibility branches are listed in `PaperInterface.lean`; the
broader certificate/iff wrapper family stays in `PostPaperAudit.lean`.
The source-law model endpoints
`theorem3_2_section3_law_optional_reporting_fairness_iff_no_test_relevance_gaussian_posterior_pbo_ae_source_law_model`
and
`theorem3_2_section3_law_report_required_fairness_iff_no_test_relevance_affine_skill_pbo_ae_source_law_model`
are the strongest current paper-facing continuous-law wrappers: they require
the Gaussian upper-tail marginal law only at nonblank relevance witnesses. The
older pointwise upper-tail source-equilibrium objects should be treated as
diagnostics or staging surfaces.
