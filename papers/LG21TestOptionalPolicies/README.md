...
| Lemma 4.1, strategy-proofness when access is observed | `paper_lemma4_1_reporting_cutoff_has_profitable_deviation_core`, `paper_lemma4_1_gaussian_reporting_cutoff_has_profitable_deviation`, `paper_lemma4_1_no_nontrivial_gaussian_reporting_cutoff_of_no_profitable_withholding`, `paper_lemma4_1_take_test_cutoff_has_profitable_deviation`, `LG21ObservedAccessStrategyProofCertificate`, `paper_lemma4_1_strategy_proofness_of_certificate` | partially formalized | `MainTheorems.lean`, `PaperInterface.lean` | The optional-reporting proof now has the continuous strictly increasing cutoff-deviation core, its Gaussian posterior-score instantiation, and a theorem that a nontrivial Gaussian reporting cutoff contradicts no-profitable-withholding. The report-required proof has the Gaussian half-probability deviation witness for skills between `q̃` and `q̄`. The final endpoint still depends on wiring these scalar no-deviation contradictions into the concrete equilibrium predicate. |
...
contradiction cores for Propositions 4.2--4.3. The Proposition 4.2 and
Proposition 4.3 cores now have concrete Gaussian posterior-score law
instantiations from conditional mean gaps and signal-precision scale gaps. It
does not yet formalize the paper's concrete strategic equilibrium model, Lemma
4.1, Theorem 3.1/3.2, or the final equilibrium/reporting bridge used in the
source proofs.
