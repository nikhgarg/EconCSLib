...
- The concrete base-mixed continuous-law route now has short Section 3
  ...
  These state hidden access and then, for every equilibrium index, produce the
...
- Theorem 3.1 now has Section 3 hidden-access wrappers over both PMF and
  ...
  access status" a theorem conjunct via
...
- Theorem 3.2 now has Section 3 hidden-access wrappers over PMF and
  ...
  routing through the event-or-blank case split.  The same concrete
  positive-share surfaces now have Section 3 fairness/no-relevance iff wrappers
  under the ordinary full-feature/base-only observable-law identities.
...
- The event-share case split now has a share-language bridge:
  ...
  The fairness-impossibility wrappers
...
- The certificate-packaged optional-reporting and report-required event-share
  ...
  source zero-share certificate routes also have direct Section 3 implication
  and no-relevance aliases in both regimes, so downstream callers can cite the
  paper's case split or the finite zero-share version directly.
...
- The optional-reporting concrete constant-latent posterior-payoff route and
  ...
  `event_or_blank` endpoints.  This pushes the paper's positive-share versus
  already-test-blank split down to the strongest concrete surfaces in both
  regimes.  The matching Section 3 no-relevance endpoints
  `paper_interface_theorem3_2_section3_optional_reporting_no_test_relevance`
  and
  `paper_interface_theorem3_2_section3_report_required_no_test_relevance`
  convert those same case-split routes into the paper's contrapositive reading:
  under fairness, no concrete base/test relevance witness can exist.
- The paper-facing final Theorem 3.2 aliases are now
