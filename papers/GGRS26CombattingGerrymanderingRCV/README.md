...
- Next-agent starting point: `GGRS26CombattingGerrymanderingRCV/START_HERE_NEXT_AGENT.md`
- Stopping-point handoff: `GGRS26CombattingGerrymanderingRCV/HANDOFF_2026-06-30_FILLED_STV_STOPPING_POINT.md`
...
|---|---|---|---|---|
| Thiele/PAV committee-score vocabulary | `PartyApprovalBallot`, `partyPAVScore`, `paper_pav_score` | formalized | `MainTheorems.lean`, `PaperInterface.lean` | None |
| PAV seat-score and min-argmax definitions | `pavSeatScore`, `pavSeatMinArgmax`, `paper_pav_seat_score`, `paper_pav_min_argmax` | formalized | `MainTheorems.lean`, `PaperInterface.lean` | None |
| Lemma C.1 interval characterization | `pavSeatInterval`, `paper_pav_seat_interval` | formalized | `MainTheorems.lean`, `PaperInterface.lean` | None |
| Lemma C.1 PAV argmax-to-interval statement | `pavSeatMinArgmax_seatInterval`, `paper_pav_min_argmax_seat_interval` | formalized | `MainTheorems.lean`, `PaperInterface.lean` | None |
| Lemma C.1 interval-to-rounding bridge | `pavSeatInterval_seatShareRounded`, `paper_pav_interval_seat_share_rounded` | formalized | `MainTheorems.lean`, `PaperInterface.lean` | None |
| Proposition 1 rounding target | `seatShareRounded`, `paper_seat_share_rounded` | formalized | `MainTheorems.lean`, `PaperInterface.lean` | None |
| Proposition 1 generated filled-seat fractional STV route | `paper_proposition1_from_generated_filled_seat_run_fractional_stv_trace_global_weight_terminal_and_pav_min_argmax` | formalized | `PaperInterface.lean` | None; the source-level row uses a total, quota-respecting fractional STV choice rule and reads the party STV result from `partyFilledSeatCount` on the generated filled-seat run. |
| Proposition 1 STV solid-coalition party isolation | `paper_stv_solid_coalition_party_trace_isolation`, `paper_stv_solid_coalition_ballots_party_trace_isolation` | formalized | `PaperInterface.lean` | None |
| Proposition 1 STV quota-capacity step | `paper_stv_quota_floors_fit` | formalized | `PaperInterface.lean` | None |
| Proposition 1 STV quota-witness bridge | `paper_stv_solid_coalition_quota_witness_bounds` | formalized | `PaperInterface.lean` | None |
| Proposition 1 STV quota-witness-to-lower-bound bridge | `paper_stv_solid_coalition_quota_witness_bounds_lower_bounds`, `paper_stv_solid_coalition_lower_bounds` | formalized | `PaperInterface.lean` | None |
| Proposition 1 STV lower-bound-to-rounding bridge | `paper_stv_solid_coalition_lower_bounds_seat_share_bounds`, `paper_stv_seat_share_bounds` | formalized | `PaperInterface.lean` | None |
| Proposition 1: STV/PAV rounded seat shares | `paper_proposition1_from_generated_filled_seat_run_fractional_stv_trace_global_weight_terminal_and_pav_min_argmax` | formalized | `PaperInterface.lean` | None; final-seat accounting is derived from the filled-seat source semantics, not assumed through `helectCount`. |
| Redistricting optimization and simulations | `none` | not formalized | `none` | Data/code boundary outside the theorem ledger |
