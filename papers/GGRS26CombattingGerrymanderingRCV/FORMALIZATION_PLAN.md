...
- Small reusable lemmas added now: the interval-to-floor/ceil rounding theorem
  ...
  canonical quota residual certificates, and terminal same-party quota-process
  constructors. The STV quota-capacity step
  `twoParty_floor_votes_div_STVQuota_sum_le_seats` is now in
  `EconCSLib.SocialChoice.Voting.STV.Quota`.
- Larger reusable components to defer: the trace/replay theorem connecting
  ballot-level fractional STV traces to the source-facing solid-coalition
  quota-process outcome predicate.
- Library-audit risks: Proposition 1 is Lean-closed for the formal
  solid-coalition STV outcome predicate. A future ballot-level replay theorem
  would strengthen the model, but no paper-local assumption declaration remains.
...
- Current Lean endpoints:
  ...
  `paper_proposition1_from_stv_bounds_and_pav_min_argmax` in
  `PaperInterface.lean`.
- Exact current mathematical gap: none for the source-shaped Appendix
  Proposition 1 proof. The deferred strengthening is a ballot-level
  fractional-STV replay theorem connecting concrete traces to
  `paper_stv_solid_coalition_process_bounds`.
- Next bridge lemmas to try if strengthening the model: a ballot-level
  two-party solid-coalition replay theorem connecting the existing
  `Voting.STV` trace skeleton to the process predicate.
