...
- The CTMC monotonicity layer now also exposes
  ...
  ratio-source wrappers for the finite-or-infinite aggregate-cross and
...
| Theorem 2, multiplicative pricing not IC and optimal policy shapes | `MultiplicativeNotICCertificate`, `paper_theorem2_multiplicative_not_ic_of_witness`, `MultiplicativePolicyShapeCertificate`, `paper_theorem2_multiplicative_policy_shape_of_certificate`, `rejectsLongTripsFiniteOrInfiniteCutoff`, `MultiplicativeExtendedPolicyShapeCertificate`, `paper_theorem2_multiplicative_extended_policy_shape_of_certificate`, `paper_theorem2_multiplicative_extended_policy_shape_of_shape_derivation`, `paper_theorem2_multiplicative_extended_measurable_policy_shape_of_shape_derivation` | conditional | `MainTheorems.lean`, `PaperInterface.lean` | The finite-cutoff certificate wrappers remain available. Lean now also states the source-faithful non-surge `t = infinity` case as accept-all and derives the multiplicative policy-shape conclusion from the Theorem 4 shape-derivation boundary when the non-surge branch is positive/decreasing and the surge branch is positive/increasing. The profitable-deviation/non-IC example is still conditional on an explicit witness. |
