...
and this README.  The importable source-numbered endpoint ledger is
`PostPaperAudit.lean`; the larger historical alias layer is retained separately
in `InterfaceAliases.lean`.
...
- The Theorem 3 denominator domain is now explicit.  `DomainBridge.lean`
  exposes the compact paper-facing endpoint as
  `PaperInterface.theorem3_defined_reward_source`: Appendix D reward rates are
  compared on the positive-denominator domain where the accepted-trip mass/time
  quotients are defined.  `DomainBridge.lean` also keeps optional totalized
  zero-mass audit lemmas.  It proves the reusable lift from positive-mass
  measurable IC/AE uniqueness to a totalized feasible-measurable statement when
  an explicit `DynamicZeroMassStrictDominanceCertificate` is supplied, and it
  proves simplification/obstruction lemmas showing why that certificate is not
  automatic under Lean's totalized real-valued CTMC reward interface.
- Theorem 3 now has explicit existential structured-price endpoints for the
  ...
  The compact paper-facing source endpoint is
  `PaperInterface.theorem3_defined_reward_source`, whose conclusion is the
  existential structured-price defined-reward IC and a.e.-unique accept-all
  statement on the positive-denominator source domain.
  ...
  `PaperInterface.theorem3_positive_mass_source_positive_response`.  Optional
  totalized feasible-measurable strengthenings remain exposed via
  `PaperInterface.theorem3_source_with_zero_mass_dominance` and
  `PaperInterface.theorem3_positive_mass_to_full_with_zero_mass_dominance`, but
  they are not the compact paper-facing Theorem 3 row.
- Theorem 4 now has compact paper-facing structural endpoints for the
  ...
  The same a.e. representative endpoint is now also exposed from the
  ...
  which proves measurable optima reject long non-surge trips and short surge
  trips up to null feasible-trip sets.  The non-IC branch now also has a
  closed explicit atomic measured counterexample exposed as
  `PaperInterface.theorem2_multiplicative_measured_not_ic_explicit_atomic`;
  it reduces the Section 5-style aggregate comparison to two weighted Dirac
  trip distributions and elementary CTMC switch-probability bounds.
  The accept-all positive-response branch used by the current Theorem 3 route
  now also has structural representative wrappers:
  ...
  The corresponding a.e.-unique accept-all optimality surface is
  `PaperInterface.theorem4_acceptAll_ae_unique_of_positive_response_candidates`.
  ...
  and the a.e. uniqueness conclusion through
  `PaperInterface.theorem4_acceptAll_ae_unique_of_optimal_current_bounds`,
  avoiding the older all-candidate nondegeneracy boundary.  The all-feasible
  current-bounds source package is also exposed as
  `PaperInterface.theorem4_acceptAll_ae_unique_of_current_bounds_source`.  The exact
  ...
  `Theorem4Frontier.lean` now also exposes the exact one-threshold
  accept-all uniqueness boundary through
  `PaperInterface.theorem4_acceptAll_unique_exact_one_threshold_non_accept_all_endpoint`,
  `PaperInterface.theorem4_acceptAll_unique_fixed_transfer_exact_one_threshold_non_accept_all_endpoint`,
  and
  `PaperInterface.theorem4_acceptAll_unique_extended_one_threshold_surge_cross_source`.
  These wrappers package the paper's non-accept-all finite-ray convention and
  the aggregate-cross fixed-transfer local endpoint data into the literal
  accept-all unique-measurable-optimum conclusion.  The a.e. representative
  route now also has an existence-based Theorem 4 frontier through
  `PaperInterface.theorem4_acceptAll_ae_unique_feasible_ae_representative_rejected_mass`,
  `PaperInterface.theorem4_acceptAll_ae_unique_fixed_response_representative_rejected_mass`,
  and
  `PaperInterface.theorem4_acceptAll_ae_unique_gn21_bracket_representative_rejected_mass`.
  These endpoints derive accept-all optimality internally from existence of an
  optimum plus positive-rejected-mass strict endpoint moves, so the source
  boundary no longer needs a separate accept-all optimality proof before
  applying the a.e. Theorem 4 conclusion.  The closest fixed-price source
  wrappers now include
  `PaperInterface.theorem4_acceptAll_ae_unique_middle_reroute_policy_form_source_existence`,
  `PaperInterface.theorem4_acceptAll_ae_unique_middle_reroute_light_eq_source_existence`,
  `PaperInterface.theorem4_acceptAll_ae_unique_light_ae_feasible_canonical_endpoint`,
  `PaperInterface.theorem4_acceptAll_ae_unique_fixed_response_eq_middle_reroute`,
  `PaperInterface.theorem4_acceptAll_ae_unique_fixed_response_bracket_eq_middle_reroute`,
  `PaperInterface.theorem4_acceptAll_ae_unique_fixed_response_by_policy_form_middle_cutoff`,
  `PaperInterface.theorem4_acceptAll_ae_unique_fixed_response_surge_cross_middle_cutoff`,
  and
  `PaperInterface.theorem4_acceptAll_ae_unique_fixed_response_surge_cutoff_middle_cutoff`:
...
- Appendix D now exposes the derivative-sign kernel, the polynomial-to-response
  ...
  `PaperInterface.theorem3_small_surge_mass_affine_current_final_sign_no_rmax_normalized`.
  The parallel fixed-lower-cross normalized endpoint
  `PaperInterface.theorem3_small_surge_mass_affine_fixed_lower_cross_no_rmax_normalized`
  now removes the explicit `Rmax` witness on the route that derives current
  lower slack from the fixed-state cross comparison instead of from a
  per-policy current final-sign field.  The still closer endpoint
  `PaperInterface.theorem3_small_surge_mass_affine_fixed_lower_cross_final_sign_no_rmax_normalized`
  also derives the accept-all lower endpoint from the paper's cross-multiplied
  Lemma 9 final-sign inequality.
...
- The finite MDP declarations are marked auxiliary support only and live in
  ...
  theorems.
- The DAG uses the shared TikZ preamble and shows the named paper theorem chain
  as verified support. Lemma 3 now has outgoing dependencies into Lemma 1 and
  the aggregate-reward/Lemma 6 pipeline, matching the paper's use of time
  fractions in the dynamic reward formula. Proposition 3.1 has a model-to-result
  edge rather than a dependency on Theorem 1, since the renewal-reward affine IC
  proof is direct. Optional zero-mass totalization is documented in the report,
  not drawn as a prerequisite for the closed Theorem 3 endpoint.
...
| Theorem 1, single-state threshold best response | `strictThresholdPolicy`, `completeThresholdPolicy`, `thresholdBoundaryPolicy`, `singleStateTripPayment_acceptAll_pos_of_rate_nonneg_of_positive_payout_mass`, `paper_theorem1_policy_le_complete_threshold_at_own_reward`, `paper_theorem1_step2_partial_threshold_dominated_by_strict_or_complete`, `paper_theorem1_high_reward_band_or_gap`, `upperSemicontinuousOn_theorem1_cutoff_objective`, `paper_theorem1_single_state_threshold_best_response_measurable_of_positive_payout_mass` | formalized | `MainTheorems.lean`, `EconCSLib/Foundations/Probability/RenewalReward.lean` | None; Lean proves the source measurable single-state threshold best response for nonnegative measurable on-trip rates with finite accept-all mass, accept-all payment/time integrability, positive arrival rate, and the source proof branch with positive mass on positive payouts. The internal positive accept-all expected-payment premise is derived from that source branch. Step 1 sends any measurable feasible policy to the complete threshold at its own renewal reward; Step 2 reduces partial thresholds to strict/complete thresholds; Step 3 obtains a compact maximizer using tail bounds, the high-reward band/gap split, and one-sided dominated convergence, so atoms on threshold boundaries are allowed. |
...
| Lemmas 9-10, IC derivative conditions | `paper_lemma9_surge_derivative_positive_of_acceptAll_bounds`, `paper_lemma10_nonsurge_derivative_positive_of_acceptAll_bounds`, `paper_lemma9_derivative_value_pos_of_current_bounds_certificate`, `paper_lemma10_derivative_value_pos_of_current_bounds_certificate`, `paper_lemma9_structured_bounds_feasible_positive_of_positive_primitives`, `paper_lemma10_structured_bounds_feasible_of_positive_pieces`, `lemma9StructuredBounds_of_acceptAll_tightening`, `lemma10StructuredBounds_of_acceptAll_tightening`, `lemma9SurgeDerivativeCertificate_of_current_bounds`, `lemma10NonsurgeDerivativeCertificate_of_current_bounds`, `lemma5DerivativeShapeWitness_positive_of_lemma9_current_bounds`, `lemma5DerivativeShapeWitness_positive_of_lemma10_current_bounds`, `gn21MeasuredAggregateRewardPrimitives_lt_acceptAll_left_of_lemma10_current_bounds`, `gn21MeasuredAggregateRewardPrimitives_lt_acceptAll_right_of_lemma9_current_bounds`, `GN21NonsurgeLemma10AcceptAllAggregateSourceData.aggregate_lt_acceptAll`, `GN21SurgeLemma9AcceptAllAggregateSourceData.aggregate_lt_acceptAll` | formalized | `MainTheorems.lean` | None; Lean proves the structured ratio intervals are feasible and that source accept-all bounds, after the formal tightening step to current `Q,T` primitives, imply positive upper-endpoint reward derivatives through the Lemma 6 derivative-value bridge. The current-bound layer also gives strict accept-all aggregate reward improvements in the surge and non-surge states whenever the rejected complement has positive measure of nonzero Lemma 6 derivative kernel, with full-data, primitive-data, and source-data wrappers. General policy endpoint selection is tracked under Lemma 5/Theorem 4, not Lemmas 9-10. |
| Theorem 4, appendix structural theorem subsuming Theorem 2 | `theorem4NonsurgeShape`, `theorem4SurgeShape`, `theorem4NonsurgeAllowedLemma5Shape`, `theorem4SurgeAllowedLemma5Shape`, `paper_theorem4_measurable_dynamic_structural_policy_of_allowed_policy_forms`, `paper_theorem4_measurable_dynamic_structural_policy_representatives_of_feasible_ae_policy_forms`, `paper_theorem4_measurable_dynamic_structural_policy_representatives_of_fixed_response_policy_forms`, `paper_theorem4_measurable_dynamic_structural_policy_representatives_of_gn21_fixed_response_source_data`, `paper_theorem4_measurable_dynamic_structural_policy_representatives_of_gn21_bracket_source_data`, `PaperInterface.theorem4_structural_policy_forms_of_allowed_policy_forms`, `PaperInterface.theorem4_structural_policy_representatives_of_feasible_ae_policy_forms`, `PaperInterface.theorem4_structural_policy_representatives_of_fixed_response_policy_forms`, `PaperInterface.theorem4_structural_policy_representatives_of_gn21_fixed_response_source_data`, `PaperInterface.theorem4_structural_policy_representatives_of_gn21_bracket_source_data` | formalized | `MainTheorems.lean`, `Lemma5Frontier.lean`, `PaperInterface.lean` | None; the source structural theorem is closed at the measure-theoretic surface: Lean proves existence of an optimal policy with the listed non-surge/surge forms and proves every measurable optimum has the corresponding a.e. structural representatives. |
| Theorem 4 accept-all uniqueness corollaries for Theorem 3 | `paper_theorem4_measurable_accept_all_ae_unique_optimal_of_positive_response_acceptAll_candidates`, `paper_theorem4_measurable_accept_all_ae_unique_optimal_of_structured_current_bounds_source_positive_response`, `paper_theorem4_measurable_accept_all_ae_unique_optimal_of_structured_current_bounds_optimal_source_positive_response`, `PaperInterface.theorem4_acceptAll_ae_unique_of_positive_response_candidates`, `PaperInterface.theorem4_acceptAll_ae_unique_of_current_bounds_source`, `PaperInterface.theorem4_acceptAll_ae_unique_of_optimal_current_bounds` | formalized | `MainTheorems.lean`, `PaperInterface.lean` | None; these are closed certificate endpoints. They are the Theorem 4 accept-all uniqueness routes used by the positive-response/current-bounds Theorem 3 proofs. |
| Optional Theorem 4 exact/representative selection corollaries | `paper_theorem4_measurable_accept_all_unique_optimal_of_endpoint_current_bounds_exact_one_threshold_non_accept_all_selection_unless`, `paper_theorem4_measurable_accept_all_unique_optimal_of_endpoint_theorem3_fixed_transfer_regular_exact_one_threshold_non_accept_all_selection_unless`, `paper_theorem4_measurable_accept_all_ae_unique_optimal_of_feasible_ae_forms_and_representative_rejected_mass_improvements_from_exists`, `paper_theorem4_measurable_accept_all_ae_unique_optimal_of_fixed_response_policy_form_by_policy_form_middle_cutoff_reroute_source_existence`, `paper_theorem4_measurable_accept_all_ae_unique_optimal_of_fixed_response_policy_form_surge_cross_by_policy_form_middle_cutoff_reroute_source_existence`, `PaperInterface.theorem4_acceptAll_unique_exact_one_threshold_non_accept_all_endpoint`, `PaperInterface.theorem4_acceptAll_ae_unique_feasible_ae_representative_rejected_mass`, `PaperInterface.theorem4_acceptAll_ae_unique_fixed_response_by_policy_form_middle_cutoff`, `PaperInterface.theorem4_acceptAll_ae_unique_fixed_response_surge_cross_middle_cutoff`, `PaperInterface.theorem4_acceptAll_ae_unique_fixed_response_surge_cutoff_middle_cutoff` | formalized | `Theorem4Frontier.lean`, `Theorem3Frontier.lean`, `PaperInterface.lean` | None; these optional frontier wrappers compile and are not part of the named Theorem 4 closure above. Deriving their exact/non-accept-all one-threshold source-selection packages, or broader a.e. representative positive-rejected-mass endpoint moves, from still more primitive assumptions remains a future strengthening rather than a named-result gap. |
| Theorem 2, multiplicative optimal policy shapes | `paper_theorem2_multiplicative_extended_measurable_policy_shape_of_shape_derivation`, `paper_theorem2_multiplicative_measurable_policy_shape_ae_of_feasible_ae_policy_forms`, `paper_theorem2_one_threshold_measurable_policy_shape_ae_of_gn21_bracket_source_data`, `PaperInterface.theorem2_multiplicative_policy_shape_ae_of_feasible_ae_policy_forms`, `PaperInterface.theorem2_one_threshold_policy_shape_ae_of_gn21_bracket_source_data` | formalized | `MainTheorems.lean`, `Lemma5Frontier.lean`, `PaperInterface.lean` | None; the a.e. optimal-policy shape clause is closed at the Theorem 4/feasible Lemma 5 handoff, with raw GN21 bracket data giving the one-threshold CTMC source surface. |
| Theorem 2, multiplicative pricing is not IC | `paper_theorem2_multiplicative_measured_not_ic_explicit_atomic`, `paper_theorem2_multiplicative_measured_profitable_deviations_in_both_states_explicit_atomic`, `paper_theorem2_multiplicative_measured_profitable_positive_finite_cutoff_deviations_in_both_states_explicit_atomic`, `paper_theorem2_multiplicative_measured_not_ic_both_states_explicit_atomic`, `theorem2AtomicMu`, `theorem2AtomicAggregate_profitable`, `theorem2AtomicQ_bounds`, `theorem2BothStatesAtomicMu`, `theorem2BothStatesAggregate_profitable_nonsurge`, `theorem2BothStatesAggregate_profitable_surge`, `MultiplicativeMeasuredAggregateNotICCertificate`, `paper_theorem2_multiplicative_measured_not_ic_of_aggregate_witness`, `PaperInterface.theorem2_multiplicative_measured_not_ic_explicit_atomic`, `PaperInterface.theorem2_multiplicative_measured_profitable_deviations_both_states_explicit_atomic`, `PaperInterface.theorem2_multiplicative_measured_profitable_positive_finite_cutoff_deviations_both_states_explicit_atomic`, `PaperInterface.theorem2_multiplicative_measured_not_ic_both_states_explicit_atomic`, `PaperInterface.theorem2_multiplicative_measured_not_ic_witness`, `PaperInterface.theorem2_multiplicative_measured_not_ic_aggregate_witness` | formalized | `MainTheorems.lean`, `Theorem2ExplicitInstance.lean`, `PaperInterface.lean` | None; Lean proves the measured multiplicative non-IC branch for explicit atomic instances. The new both-state witness uses trip masses split equally at lengths `1` and `6` in both states, arrivals `1` and `2`, switch rates `1/4` and `1/4`, and multiplicative rates `1` and `10`. In that same measured instance, accept-all is strictly beaten by a non-surge reject-long deviation with positive finite cutoff `2` and by a surge reject-short deviation with positive finite cutoff `2`; the positive-finite cutoff facts are stated explicitly in the paper-facing theorem. The proof reduces `Q,T,W` primitives to weighted-Dirac integrals, bounds the CTMC switch probabilities, proves both strict aggregate reward improvements, and feeds the existing measured aggregate certificate/dynamic reward bridge into dynamic non-IC. |
| Theorem 3, structured IC pricing | `paper_theorem3_measured_structured_positive_mass_measurable_ic_ae_unique_prices_of_source_assumptions`, `paper_theorem3_measured_structured_defined_reward_ic_ae_unique_prices_of_source_assumptions`, `theorem3MeasuredStructuredPositiveMassMeasurableICAEUniqueConclusion`, `theorem3MeasuredStructuredDefinedMeasurableICAEUniqueConclusion`, `PaperInterface.theorem3_positive_mass_source`, `PaperInterface.theorem3_defined_reward_source`, `theorem3MeasuredStructuredMeasurableICAEUniqueConclusion_of_positiveMass_ae_unique_and_zeroMassStrictDominance`, `PaperInterface.theorem3_zero_mass_dominance_of_acceptAll_dominates_zero_mass`, `PaperInterface.theorem3_zero_mass_totalization_obstruction_state_rates`, `PaperInterface.theorem3_zero_mass_dominance_impossible_of_profitable_zero_mass` | formalized | `DomainBridge.lean`, `MainTheorems.lean`, `Theorem3SplitCurrentBounds.lean`, `ProofInterface.lean`, `PaperInterface.lean` | None for the paper-facing endpoint; Lean closes the denominator-valid defined-reward source theorem that matches Appendix D reward-rate semantics. `PaperInterface.theorem3_defined_reward_source` uses the sequential Lemma 9 then Lemma 10 source route, constructs structured prices, proves defined-reward IC, and proves a.e. accept-all uniqueness for defined-reward optima. Optional totalized zero-mass bridges remain exposed separately, and Lean also proves obstruction lemmas showing that a profitable zero-mass deviation rules out the totalization certificate once accept-all is positive-mass optimal. |
...
The stronger source-ordered implementation helper reuses the existing feasible
sequential current-bounds source certificate directly:
...
then proves non-surge accepts all a.e.  The paper-facing theorem row is the
defined-reward endpoint `PaperInterface.theorem3_defined_reward_source`.  The
lower-level compact bridges are
...
The current source-faithful paper-facing Theorem 3 frontier is
`PaperInterface.theorem3_defined_reward_source`.  The feasible sequential
current-bounds source-data route remains an implementation helper when the
source proof supplies the policy-dependent sequential Lemma 9/10 source
packages already used by the IC-only theorem.  If weak feasible IC is separate
and Lemma 9/10 source data are
...
directly.  The source-facing fixed-reward-rate variant
