...
- Appendix D now exposes the derivative-sign kernel, the polynomial-to-response
  ...
  per-policy current final-sign field.
...
| Theorem 3, structured IC pricing | `paper_theorem3_measured_structured_positive_mass_measurable_ic_ae_unique_prices_of_source_assumptions`, `theorem3MeasuredStructuredPositiveMassMeasurableICAEUniqueConclusion`, `PaperInterface.theorem3_positive_mass_source`, `PaperInterface.theorem3_small_surge_mass_affine_fixed_lower_cross_no_rmax_normalized`, `theorem3MeasuredStructuredMeasurableICAEUniqueConclusion_of_positiveMass_ae_unique_and_zeroMassStrictDominance`, `PaperInterface.theorem3_zero_mass_totalization_obstruction` | formalized (positive-mass source domain) | `DomainBridge.lean`, `MainTheorems.lean`, `Theorem3Frontier.lean`, `ProofInterface.lean`, `PaperInterface.lean` | The denominator-valid source theorem is closed on the positive-mass measurable domain. Lean constructs the structured CTMC prices, proves positive-mass measurable IC, proves surge accept-all a.e. from Lemma 9, transports the measured reward through the a.e. surge replacement, and proves non-surge accept-all a.e. from Lemma 10 with surge fixed at exact accept-all. The split frontier also exposes the no-`Rmax` mass-affine fixed-lower-cross source route for the small-surge proof path. The stronger full feasible-measurable lift is optional and available only with an explicit `DynamicZeroMassStrictDominanceCertificate` or a revised non-totalized reward interface, because the paper reward-rate formulas divide by accepted trip mass; Lean also proves the left-empty/right-accept-all totalization obstruction for the current real-valued reward model. |
