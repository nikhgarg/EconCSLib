...
| Theorem 3, structured IC pricing | `paper_theorem3_measured_structured_positive_mass_measurable_ic_ae_unique_prices_of_source_assumptions`, `theorem3MeasuredStructuredPositiveMassMeasurableICAEUniqueConclusion`, `PaperInterface.theorem3_positive_mass_source`, `theorem3MeasuredStructuredMeasurableICAEUniqueConclusion_of_positiveMass_ae_unique_and_zeroMassStrictDominance` | formalized with source-domain caveat | `DomainBridge.lean`, `MainTheorems.lean`, `ProofInterface.lean`, `PaperInterface.lean` | The denominator-valid source theorem is closed on the positive-mass measurable domain. Lean constructs the structured CTMC prices, proves positive-mass measurable IC, proves surge accept-all a.e. from Lemma 9, transports the measured reward through the a.e. surge replacement, and proves non-surge accept-all a.e. from Lemma 10 with surge fixed at exact accept-all. Full feasible-measurable lifting is available only with an explicit `DynamicZeroMassStrictDominanceCertificate` or a revised non-totalized reward interface, because the paper reward-rate formulas divide by accepted trip mass. |
