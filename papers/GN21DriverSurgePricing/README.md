...
| Lemma 4, single-state optimal interval form | `agreesAwayFromThresholdBoundary`, `SingleStateUniqueThresholdCertificate`, `paper_lemma4_reward_cutoff_complete_threshold_optimal`, `paper_lemma4_unique_threshold_certificate_of_complete_threshold_optimal`, `paper_lemma4_single_state_unique_threshold_of_complete_threshold_optimal`, `paper_lemma4_single_state_unique_threshold_of_certificate` | conditional | `MainTheorems.lean` | Lean now proves the first source step of Lemma 4: from any optimal complete-threshold policy, moving the cutoff to the realized reward rate gives another optimal complete-threshold policy with reward exactly equal to its cutoff. The remaining explicit source obligation is uniqueness away from boundary trips `w(tau)/tau = c` after Theorem 1's compactness/continuity argument supplies a complete-threshold optimum. |
