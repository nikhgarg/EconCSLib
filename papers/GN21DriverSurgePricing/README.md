...
- Theorem 4 now has compact paper-facing structural endpoints for the
  ...
  accept-all unique-measurable-optimum conclusion.  The remaining analytic
  Theorem 4 work is to derive those exact/non-accept-all one-threshold source
  selection and fixed-state aggregate-cross endpoint facts from the paper
  primitives for arbitrary measurable optima, not to reassemble the
