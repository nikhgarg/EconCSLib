...
- The zero-mass boundary is now explicit.  `DomainBridge.lean` proves the
  ...
  to the other state's reward rate.  The paper-facing review surface records
  this as `PaperInterface.theorem3_zero_mass_totalization_obstruction`, backed
  by the concrete simplifications
  `gn21MeasuredDynamicReward_eq_right_rewardRate_of_left_empty_acceptAll` and
  ...
  showing that a profitable zero-mass deviation rules out the zero-mass
  dominance certificate once accept-all is optimal on the positive-mass source
  domain.  Thus source-faithful reward-rate routes should either use the
  compiled feasible sequential current-bounds source-data endpoint, stay on the
  positive-mass/nondegenerate domain, or use the compiled partial interface
  `PaperInterface.theorem3_defined_reward_ic_of_positive_mass`, where zero-mass
  denominator failures have no reward value.  The shortest compiled
  accept-all route to that condition is
...
- The DAG uses the shared TikZ preamble and shows the named paper theorem chain
  ...
  proof is direct. The zero-mass reward audit is drawn as a separate downstream
  result, not as a prerequisite for the closed Theorem 3 endpoint.
