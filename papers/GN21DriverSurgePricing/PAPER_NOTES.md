...
- Last theorem row verified: Theorem 1 measured marginal add/remove steps,
  ...
  Theorem 3 fixed-transfer local data packages
  ...
  mass-separated pointwise/reward-rate endpoint certificate
  ...
  with its `to_pointwise_reward_rate` adapter, source wrappers
  ...
  and shared-source fixed-state mass helpers
  ...
  policy-form fixed-state data records
  ...
  positive-tail-to-uniform-tail infrastructure
  `integrableOn_Ioi_of_intervalIntegrable_of_integrableOn_Ioi`,
  `GN21TailProductIntegrabilityData.of_interval_and_tail`,
  ...
  `GN21SurgeTheorem3FixedTransferPositiveTailData`,
  `GN21SurgeTheorem3FixedTransferPositiveTailData.of_shared_acceptAll`, and
  ...
  fixed-state-by-policy-form derived-tail endpoint certificate
  ...
  with its `to_positive_tail` adapter, the positive-tail endpoint certificate
