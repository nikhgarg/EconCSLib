...
| Proposition 2 square-root representation bridge | `paper_uniform_sqrt_homogeneity_of_count_closeness` | formalized bridge | `AccuracyDiversity/MainTheorems.lean` | requires the still-unproved real-relaxation/integer-rounding count-closeness bound |
| Appendix D.5 rounding combinatorics | `paper_rounding_count_close_of_no_crossing` | formalized bridge | `AccuracyDiversity/MainTheorems.lean` | requires the analytic proof that optimality rules out high/low crossings around the real-relaxation floor anchors |
...
For Proposition 2, the uniform `k = 1` marginal formulas and the square-root
homogeneity representation bridge are formalized. The combinatorial half of
Appendix D.5 is also formalized; the remaining mathematical work is the
real-relaxation optimizer and the analytic proof that its optimality rules out
high/low integer crossings.
