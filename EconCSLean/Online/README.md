...
6. Section 7 now has the finite Yao/minimax expectation lemma, the uniform
   ...
   now needs only the deterministic round-allocation inequalities. The closest
   concrete endpoint is `BMatchingTheorem9IntegralPrefixChoiceFamily`, for
   finite deterministic choice-rule families whose payoff is the capped
   normalized spend expression; only a separate actual-revenue-to-capped-spend
   comparison is needed for richer algorithm models.
