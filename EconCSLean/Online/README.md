...
3. The stepwise/primal-dual charging argument is formalized for the finite
   ...
   `paper_adwords_balance_msvv_approx_competitive_with_error_bound`; the
   delta-form wrapper is
   `paper_adwords_balance_msvv_approx_competitive_up_to_delta`. The ideal exact
   seam `MsvvHistoryAccountingCertificate` remains available for a zero-error
   limiting theorem.
