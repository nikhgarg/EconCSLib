...
/--
LOS02 generalized Vickrey auction generated by a supplied allocation rule.
...
/-- LOS02 Theorem 4.1: a welfare-maximizing GVA is truthful. -/
...
/--
LOS02 Proposition 4.2: under nonnegative bundle values, a truthful bidder's GVA
...
/--
The source reduction behind LOS02 Theorem 6.1: feasibility in weighted set
packing is exactly feasibility of the corresponding single-minded accepted set.
-/
...
/-- LOS02 Participation: denied bidders pay zero. -/
...
/--
LOS02 Monotonicity: a granted bid remains granted after asking for a subset and
...
/--
Domain-restricted monotonicity. This is the version used for the paper's
nonempty, nonnegative single-minded type space: the strengthened report must
remain inside the admissible domain.
-/
...
/--
LOS02 Critical-value data. The threshold may depend on other reports and the
...
/--
LOS02 Theorem 9.6, source-shaped form for accepted-set mechanisms over
...
/--
LOS02 critical-value data with an explicit infinite case. A finite threshold
...
/--
Finite thresholds are monotone in the LOS02 bundle order even when smaller
...
/--
LOS02 Theorem 9.6 with the source's infinite critical-value branch represented
by `none`. Exactness is represented by the accepted-set allocation model.
-/
...
/--
LOS02 Lemma 9.1 threshold existence on the source domain. For a fixed nonempty
...
threshold is the separate Critical axiom used in Theorem 9.6.
-/
...
/--
LOS02 critical-value data for the nonnegative single-minded report domain. The
finite and infinite threshold clauses only need to govern admissible
nonnegative reports, which matches the paper's type-space restriction and avoids
...
/--
Domain-aware version of LOS02 Theorem 9.6. Critical-value clauses are only
required on the nonempty, nonnegative single-minded report domain used by the
paper, while exactness is still represented by the accepted-set allocation
model.
-/
...
/-- The square-root norm used in LOS02 Theorem 7.2, `a / sqrt(|s|)`. -/
...
under the LOS02 set-to-bid encoding.
-/
...
/--
The LOS02 set-to-bid encoding preserves any multiplicative approximation
...
/-- The LOS02 set-to-bid encoding as a map of threshold decision instances. -/
...
/--
The LOS02 set-to-bid encoding as an abstract many-one reduction from weighted
...
/--
If an external complexity model certifies the LOS02 set-to-bid encoding as
...
for weighted set packing via the LOS02 encoding.
-/
...
/--
Sum of squared LOS02 square-root norms, written without square roots:
`(a / sqrt(|s|))^2 = a^2 / |s|`.
-/
...
/--
Common-bid removal algebra for LOS02 Theorem 7.2. If the approximation
inequality is known after deleting bids common to `optimal` and `greedy`, then it
lifts back to the original sets for any approximation factor at least one.
-/
...
/--
If the LOS02 square-root norm of `i` is no larger than that of `j`, then the
...
This is the value-only perturbation used in LOS02 Theorem 10.2's critical-price
argument.
-/
...
/--
One step of the LOS02 greedy allocation: accept the next bid iff it does not
...
/-- Run the LOS02 greedy allocation from an arbitrary accepted-set state. -/
...
/-- The LOS02 greedy accepted set from an explicit bid order. -/
...
/-- The allocation induced by the LOS02 greedy accepted set. -/
...
/--
LOS02 Definition 10.1 payment formula, parameterized by the source proof's
`n(j)` function: the first later bid denied because of granted bid `j`.
-/
...
/--
Boolean test for the Definition 10.1 condition that bid `i` is denied because
...
/--
The Definition 10.1 `n(j)` candidate search inside a suffix known to follow
...
/--
Boolean prefix-state version of the Definition 10.1 condition. Bid `i` is
...
/--
Prefix-state version of the Definition 10.1 condition that bid `i` is denied
...
/--
Bid `i` is a Definition 10.1 denied-because-of candidate somewhere in a suffix,
...
/--
If the stateful Definition 10.1 search returns `n`, then no earlier bid in a
...
/--
Definition 10.1 `n(j)` from an explicit split of the sorted order into bids
...
/--
Definition 10.1 `n(j)` search over a full sorted order, carrying the greedy
...
/-- Definition 10.1 `n(j)` from the full sorted greedy order. -/
...
/--
Full-order version of the Definition 10.1 denied-because-of relation. It
...
/--
Definition 10.1 payment rule computed directly from the full sorted greedy
order.
-/
...
/--
The LOS02 greedy accepted-set mechanism induced by an order rule. The order rule
is kept explicit so theorem statements can talk about the paper's sorted order
...
/-- Denied bidders pay zero in the LOS02 greedy accepted-set mechanism. -/
...
/-- The LOS02 square-root norm is weakly descending along the explicit order. -/
...
/-- The LOS02 Section 10 average amount per good is weakly descending along an order. -/
...
/--
Deterministic key for the concrete LOS02 average-density order. The primary
...
/-- The concrete LOS02 greedy order: bidders sorted by decreasing average amount per good. -/
...
/--
The concrete LOS02 average order is stable off the bidder whose value changes:
...
/--
The ordered-insertion suffix window used by the concrete LOS02 average order
...
/--
The split-order Definition 10.1 search only returns bids that occur after `j`
...
/--
The full-order Definition 10.1 search only returns bids after the first
...
/--
Local order-window package for the finite `n(j) = n` branch of LOS02
Definition 10.1. It records the sorted window used for value-only perturbations
...
/--
Source-window data for the accepted-bid critical branch of LOS02 Definition
...
windows for the finite and no-next cases, the actual Definition 10.1 payment is
...
/--
Build the domain-aware critical-value certificate for the LOS02 greedy
...
/--
LOS02 Theorem 10.2 for the concrete average-density greedy accepted-set/payment
...
/--
The source-shaped blocking certificate used in LOS02 Theorem 7.2. Each optimal
bid is assigned to a greedy bid that blocks it on a shared good and has at least
as large a squared square-root norm.
-/
...
/--
Order-level LOS02 blocking certificate. This keeps the source proof's
statement that the blocking greedy bid appears earlier in the sorted list and
has weakly larger square-root norm.
-/
...
/-- Convert the source-order LOS02 blocking certificate to the algebraic form. -/
...
/--
The LOS02 blocking association implies the paper's blocking inequality. The
proof injects optimal bids into `(greedy bid, blocked good)` pairs. Pairwise
...
/--
LOS02 Theorem 7.2, algebraic core in squared form. If the greedy analysis gives
the paper's blocking inequality
...
/--
LOS02 Theorem 7.2, square-root approximation form derived from the same
...
/--
LOS02 Theorem 7.2 in certificate form. The remaining greedy-order proof can
focus on constructing `SingleMindedGreedyBlockingCertificate` from an actual
sorted greedy execution.
-/
...
/--
LOS02 Theorem 7.2 from the source-order blocking certificate: every optimal bid
has an earlier conflicting greedy blocker with weakly larger square-root norm.
-/
...
/--
Disjoint-case LOS02 Theorem 7.2 for the explicit sorted greedy run. This is the
source proof after the paper's reduction that removes bids common to the greedy
...
/--
LOS02 Theorem 7.2 for the explicit sorted greedy run. This theorem formalizes
the paper's common-bid removal step by filtering the order to bids disjoint from
the goods used by bids common to the optimal and greedy accepted sets, applies
the disjoint reduced theorem, and lifts the bound back to the original sets.
-/
