...
noncomputable def averageTypePolicy {m n K : ℕ} [NeZero K]
  ...
  fun k =>
    DecisionCore.Policy.averageOn ρ (fiberUsers τ k)
...
theorem averageTypePolicy_apply_toReal {m n K : ℕ} [NeZero K]
  ...
  exact DecisionCore.Policy.averageOn_apply_toReal ρ (fiberUsers τ k)
...
theorem rawUserUtility_symmetrizedPolicy_eq_average
  ...
  unfold RecommendationModel.rawUserUtility DecisionCore.Policy.agentScore
    DecisionCore.pmfExp
  calc
...
theorem userFairness_le_userFairness_symmetrizedPolicy
  ...
  classical
  unfold RecommendationModel.userFairness DecisionCore.finiteMin
  apply Finset.le_inf'
  intro u _
...
theorem itemFairness_symmetrizedPolicy_eq {m n K : ℕ} [NeZero n] [NeZero K]
  ...
  unfold RecommendationModel.itemFairness DecisionCore.finiteMin
